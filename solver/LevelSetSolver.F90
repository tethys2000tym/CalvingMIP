...
!******************************************************************************
! *
! *  Modified and adapted to solve include calving rates over directions 1 and 2
...
!------------------------------------------------------------------------------
     ...
     REAL(KIND=dp), ALLOCATABLE ::CalElemVelo(:,:), ElemVelo(:,:), SurfaceFlux(:),IceElemVelo(:,:)

     SAVE LocalMassMatrix,LocalStiffMatrix,Load, &
         TimeForce, LocalForce, ElementNodes,AllocationsDone, &
         IceElemVelo,CalElemVelo, ElemVelo, Surf, SurfaceFlux
...
!------------------------------------------------------------------------------
         ...
         CalElemVelo(1,1:n) = ListGetReal( Material,'Calving Velocity 1',n,NodeIndexes,GotIt)
         CalElemVelo(2,1:n) = ListGetReal( Material,'Calving Velocity 2',n,NodeIndexes,GotIt)
         IF(dim == 3) CalElemVelo(3,1:n) = ListGetReal( Material,'Calving Velocity 3',n,NodeIndexes,GotIt)

         IceElemVelo(1,1:n) = ListGetReal( Material,'Ice Velocity 1',n,NodeIndexes,GotIt)
         IceElemVelo(2,1:n) = ListGetReal( Material,'Ice Velocity 2',n,NodeIndexes,GotIt)
         IF(dim == 3) CalElemVelo(3,1:n) = ListGetReal( Material,'Ice Velocity 3',n,NodeIndexes,GotIt)
...
!------------------------------------------------------------------------------
         CALL LocalMatrix( LocalMassMatrix, LocalStiffMatrix, LocalForce, Surf, &
             SurfaceFlux, CalElemVelo, IceElemVelo, ElemVelo, Stabilize, CurrentElement, n, ElementNodes )
...
CONTAINS
   ...
   SUBROUTINE LocalMatrix( MassMatrix,StiffMatrix,ForceVector,  &
      Surf, Flux, CalNodalVelo, IceNodalVelo, NodalVelo, Stabilize,Element,n,Nodes )
!------------------------------------------------------------------------------

     REAL(KIND=dp), DIMENSION(:)   :: ForceVector,Surf,Flux
     REAL(KIND=dp), DIMENSION(:,:) :: MassMatrix,StiffMatrix,NodalVelo
     REAL(KIND=dp), DIMENSION(:,:) :: IceNodalVelo, CalNodalVelo
     LOGICAL :: Stabilize
     INTEGER :: n
     TYPE(Nodes_t) :: Nodes
...
!
     ...
     REAL(KIND=dp) :: Velo(3),Force,Grad(3),GradAbs,CalVelo(3), IceVelo(3)
...
!------------------------------------------------------------------------------
!      Calving Velocity from previous iteration at the integration point
!------------------------------------------------------------------------------
       CalVelo = 0.0d0
       CalVelo(1) = SUM( CalNodalVelo(1,1:n)*Basis(1:n) * SIGN(1.0d0,Nodes % x(1:n)*Basis(1:n)))
       CalVelo(2) = SUM( CalNodalVelo(2,1:n)*Basis(1:n) * SIGN(1.0d0,Nodes % x(1:n)*Basis(1:n)))
       IF ( dim > 2 ) CalVelo(3) = SUM( CalNodalVelo(3,1:n)*Basis(1:n) * SIGN(1.0d0,Nodes % x(1:n)*Basis(1:n)))

!------------------------------------------------------------------------------
!      Ice Velocity from previous iteration at the integration point
!------------------------------------------------------------------------------
       IceVelo = 0.0d0
       IceVelo(1) = SUM( IceNodalVelo(1,1:n)*Basis(1:n) * SIGN(1.0d0,Nodes % x(1:n)*Basis(1:n)))
       IceVelo(2) = SUM( IceNodalVelo(2,1:n)*Basis(1:n) * SIGN(1.0d0,Nodes % x(1:n)*Basis(1:n)))
       IF ( dim > 2 ) IceVelo(3) = SUM( IceNodalVelo(3,1:n)*Basis(1:n) * SIGN(1.0d0,Nodes % x(1:n)*Basis(1:n)))
...
!------------------------------------------------------------------------------
         VNorm = SQRT( SUM(Velo(1:dim)**2) &
                 + SUM(CalVelo(1:dim)**2) &
                 + SUM(IceVelo(1:dim)**2) &
                 -2*IceVelo(1)*CalVelo(1) &
                 -2*IceVelo(2)*CalVelo(2) &
                 -2*Velo(1)*CalVelo(1) &
                 -2*Velo(2)*CalVelo(2) &
                 -2*Velo(1)*IceVelo(1) &
                 -2*Velo(2)*IceVelo(2) &
...
!------------------------------------------------------------------------------
         DO p=1,N
           SU(p) = 0.0d0
           DO i = 1,dim
             SU(p) = SU(p) + dBasisdx(p,i) * ( Velo(i)  &
                     + (IceVelo(i) + FL - CalVelo(i)) * Grad(i)) 
           END DO

           SW(p) = 0.0d0
           DO i = 1,dim
             SW(p) = SW(p) + dBasisdx(p,i) * (Velo(i) &
                    + (IceVelo(i) + FL - CalVelo(i)) * Grad(i)) 
           END DO
         END DO
       END IF
...
!------------------------------------------------------------------------------
           ...
           DO i=1,dim
             A = A +( Velo(i) + ( IceVelo(i) - CalVelo(i) ) * Grad(i))  &
